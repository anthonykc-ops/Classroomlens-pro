...
// frontend also blocks all app access behind plan selection (no free trial),
// but THIS is what actually decides yes/no, since client-side gating is not
// a real paywall — e.g. a canceled subscription must be caught here even if
// the frontend's cached billing state hasn't refreshed yet.
export default async function handler(req, res) {
  ...
  try {
    ...
    if ((billing.plan === "monthly" || billing.plan === "annual") && billing.subscription_status === "active") {
